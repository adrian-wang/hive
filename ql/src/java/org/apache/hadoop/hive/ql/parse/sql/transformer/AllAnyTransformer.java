...
public class AllAnyTransformer extends BaseSqlASTTransformer {
  ...
  void trans(CommonTree tree, TranslateContext context) throws SqlXlateException {
    ...
    if (tree.getType() == PantheraParser_PLSQLParser.GREATER_THAN_OP || tree.getType() == PantheraParser_PLSQLParser.GREATER_THAN_OR_EQUALS_OP
        || tree.getType() == PantheraParser_PLSQLParser.LESS_THAN_OP || tree.getType() == PantheraParser_PLSQLParser.LESS_THAN_OR_EQUALS_OP
        || tree.getType() == PantheraParser_PLSQLParser.NOT_EQUAL_OP || tree.getType() == PantheraParser_PLSQLParser.EQUALS_OP ) {
      if (tree.getChild(0).getType() == PantheraParser_PLSQLParser.SQL92_RESERVED_ALL
          || tree.getChild(0).getType() == PantheraParser_PLSQLParser.SQL92_RESERVED_ANY
          || tree.getChild(0).getType() == PantheraParser_PLSQLParser.SOME_VK) {
        throw new SqlXlateException((CommonTree) tree.getChild(0), "All/Any/Some sub-query can only show at right operand of a compare operator.");
      }
      if (tree.getChild(1).getType() == PantheraParser_PLSQLParser.SQL92_RESERVED_ALL
          || tree.getChild(1).getType() == PantheraParser_PLSQLParser.SQL92_RESERVED_ANY
          || tree.getChild(1).getType() == PantheraParser_PLSQLParser.SOME_VK) {
        transAllAny(tree, context);
      }
    }
  }
  ...
  private void transForAll(CommonTree tree, String aggregationName, TranslateContext context) throws SqlXlateException {
    ...
    if(all.getChildCount()>1){
      //FIXME what if all has more than 1 subQuery node?
      throw new SqlXlateException(all, "we do not support multi-query in all, try to convert it to single query with subquery.");
    }
  ...
  private static CommonTree addALevel(CommonTree tree, TranslateContext context) throws SqlXlateException {
    ...
    CommonTree from = FilterBlockUtil.createSqlASTNode(select, PantheraParser_PLSQLParser.SQL92_RESERVED_FROM, "from");
